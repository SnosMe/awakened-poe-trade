    ...
    eng: {
        incomingOffer: {
            ...
            parse: (text: string) => {
                ...
                const player = text.substring(text.indexOf(AT_FROM) + AT_FROM.length, text.indexOf(HI_I_WOULD_LIKE_TO_BUY_YOUR));

                const item = text.substring(text.indexOf(HI_I_WOULD_LIKE_TO_BUY_YOUR) + HI_I_WOULD_LIKE_TO_BUY_YOUR.length, text.indexOf(LISTED_FOR));

                const price = text.substring(text.indexOf(LISTED_FOR) + LISTED_FOR.length, text.indexOf(IN));

                const p_stash_tab_index = text.indexOf(P_STASH_TAB);
                const league = text.substring(text.indexOf(IN) + IN.length, p_stash_tab_index != -1 ? p_stash_tab_index : text.length);
...
class TradeManager {
    ...
    private lastClipboardValue: string = null;

    constructor() {
        this.debounced_readLastLines = debounce(this.readLastLines, DEBOUNCE_READ_RATE_MS);
    }
    ...
    async start(): Promise<boolean> {
        ...
        if (platform() == 'win32') {
            this.logFilePath = `${poeProc.bin.substring(0, poeProc.bin.lastIndexOf("\\"))}\\logs\\Client.txt`;
        } else {
            this.logFilePath = `${poeProc.bin.substring(0, poeProc.bin.lastIndexOf("/"))}/logs/Client.txt`;
        }
    ...
    private pollClipboard(): string {
        ...
        if (text && text != "�" && (this.lastClipboardValue === null || text !== this.lastClipboardValue)) {
    ...
    private readLastLines(): void {
        stat(this.logFilePath, (err, fileInfos) => {
            open(this.logFilePath, 'r', (err, fd) => {
                const buffer: Buffer = Buffer.alloc(fileInfos.size - this.lastFilePosition);
                ...
                read(fd, buffer, 0, buffer.length, this.lastFilePosition, (err, bytesRead, buffer) => {
                    ...
                    lines.split(EOL)
                        .forEach(l => this.parse(l));
                });
            });
        });
    }

    private parseLine(parser: Parser, line: string): { ok: boolean, offer: Offer } {
    ...
    private parse(line: string, fromLog: boolean = true): void {
        const result = this.parseLine(fromLog ? PARSING.eng.incomingOffer : PARSING.eng.outgoingOffer, line);

        if (result.ok) {
            console.log(fromLog ? 'Incoming offer: ' : 'Outgoing offer: ', result.offer);
        }
    }
}

<template>
  <div class="max-w-md p-2">
    <div class="mb-2" v-if="!leagues.error.value">
      ...
      <template v-else-if="leagues.list.value.length">
        ...
        <div class="flex gap-x-2 mb-4">
          <div class="text-content-muted">{{ t('settings.private_league') }}</div>
          <input v-model="customLeagueId" placeholder="My League (PL12345)" class="rounded bg-surface-base px-1 mb-1 flex-1" />
        </div>
      </template>
    </div>
    ...
    <div class="mb-2">
      ...
      <div class="mb-4">
        <input v-model="accountName" class="rounded bg-surface-base px-1 block w-full mb-1 font-poe" />
      </div>
    </div>
    <div class="mb-2">
      ...
      <div class="mb-4 italic text-content-muted">{{ t(':highlight_hint') }}</div>
    </div>
    <div class="mb-2">
      ...
      <div class="mb-4 flex">
        <div class="flex mr-6">
          ...
          <input v-model.number="searchStatRange" class="rounded bg-surface-base px-1 block w-16 mb-1 font-poe text-center" />
    ...
    <ui-checkbox class="mb-4"
      v-model="requestPricePrediction">{{ t(':show_prediction') }} <span class="bg-surface-hover px-1 rounded">www.poeprices.info</span></ui-checkbox>
    <ui-checkbox class="mb-4"
      v-model="showCursor">{{ t(':cursor_pos') }}</ui-checkbox>
    <div class="mb-4" :class="{ 'p-2 bg-warn rounded': builtinBrowser }">
    ...
    <div class="border-2 rounded border-line mb-2">
      <div class="bg-surface-hover p-2 mb-2">{{ t(':warn_expensive') }}</div>
      ...
      <div class="mb-2 mx-2">
        ...
        <div class="mb-4 flex">
          <ui-toggle v-if="hotkeyQuick"
            ...
            <span class="bg-surface-base text-content-muted rounded px-2">{{ hotkeyQuick }}</span>
          </ui-toggle>
          <ui-toggle v-if="hotkeyLocked"
            v-model="lockedInitialSearch">
            <span class="bg-surface-base text-content-muted rounded px-2">{{ hotkeyLocked }}</span>
          </ui-toggle>
        </div>
      </div>
      <div class="mb-2 mx-2">
        ...
        <div class="flex">
          <div class="flex mr-6">
            <input v-model.number="apiLatencySeconds" class="rounded bg-surface-base px-1 block w-16 mb-1 font-poe text-center" />

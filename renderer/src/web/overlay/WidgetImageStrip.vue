<template>
  <Widget :config="config" v-slot="{ isEditing, isMoving }" move-handles="top-bottom">
    <div class="widget-default-style p-1" style="min-width: 5rem;">
      <div v-if="!isEditing" class="text-content-primary m-1 leading-4 text-center">{{ config.wmTitle || 'Untitled' }}</div>
      <input v-else
        class="leading-4 rounded text-content-primary p-1 bg-surface-hover w-full mb-1"
      ...
      <DndContainer tag="div" class="flex gap-x-1"
        ...
        <template #item="{ element: img }">
          <div :class="$style.card">
            ...
            <button v-if="isEditing" @click="remove(img.id)"
              class="bg-surface-raised absolute top-0 right-0 rounded-bl text-danger-text leading-none px-2 py-1 flex"><i class="fas fa-times"></i></button>
            <button v-if="isEditing" data-qa="drag-handle"
              class="bg-surface-base absolute rounded text-content-muted leading-none w-8 h-8 flex justify-center items-center cursor-move" style="top: calc(50% - 1rem); left: calc(50% - 1rem);"><i class="fas fa-arrows-alt"></i></button>
          </div>
        </template>
        <template #footer v-if="isEditing">
          <div :class="$style.card" class="flex justify-center items-center">
            ...
            <label class="text-content-muted hover:bg-surface-hover py-1 px-2 rounded cursor-pointer" for="file"><i class="fas fa-file-import"></i> {{ t('choose_file') }}</label>
          </div>
        </template>
      </DndContainer>
    </div>
